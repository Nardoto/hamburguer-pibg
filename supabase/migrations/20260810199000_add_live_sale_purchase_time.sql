...
begin
  select * into sale from public.sales_events
...
create or replace function public.team_live_sale_status()
returns table(
  id uuid, name text, event_date date, product_name text, public_token text,
  price_cents integer, stock_total integer, reserved_quantity integer, confirmed_quantity integer,
  last_purchase_at timestamptz
...
revoke all on function public.live_sale_status(text) from public;
revoke all on function public.team_live_sale_status() from public;
grant execute on function public.live_sale_status(text) to anon, authenticated;
grant execute on function public.team_live_sale_status() to authenticated;
